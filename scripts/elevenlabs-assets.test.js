...
test("music requests use the ElevenLabs compose shape", () => {
  ...
  const request = buildRequest({
    ...
    kind: "music",
    modelId: "music_v2",
    prompt: "A restrained instrumental menu loop",
    parameters: { music_length_ms: 40000, force_instrumental: true }
  }, root, { jobs: {} });
  assert.deepEqual(request, {
    prompt: "A restrained instrumental menu loop",
    model_id: "music_v2",
...
test("music generation downloads the synchronous MP3 response", async () => {
  const root = workspace();
  const value = validateManifest({
    schemaVersion: 1,
    stagingDirectory: "artifacts/elevenlabs",
    ...
    jobs: [{
      ...
      clientOutput: "client/public/generated/menu-theme.mp3"
    }]
  }, root);
  const fetchImpl = async (url, options) => {
    assert.match(url, /\/music\?output_format=mp3_48000_192$/);
    const body = JSON.parse(options.body);
    assert.equal(body.music_length_ms, 40000);
  ...
  assert.equal(state.jobs["menu-theme"].generationId, "song-123");
  assert.equal(state.jobs["menu-theme"].contentMimeType, "audio/mpeg");
  assert.equal(fs.readFileSync(state.jobs["menu-theme"].stagedPath, "utf8"), "mp3-result");
});
