const { stableHash } = require("./canonical");

let cardResolvers = {
  getCollectorVariantById: () => null,
  getGameplayCardById: () => null
};

function configureReplayCardResolvers(resolvers = {}) {
  cardResolvers = {
    getCollectorVariantById: typeof resolvers.getCollectorVariantById === "function"
      ? resolvers.getCollectorVariantById
      : () => null,
    getGameplayCardById: typeof resolvers.getGameplayCardById === "function"
      ? resolvers.getGameplayCardById
      : () => null
  };
}
...
function exactEvidenceCard(cardId) {
  const gameplay = cardId ? cardResolvers.getGameplayCardById(cardId) : null;
