...
function choosePaymentIds(hand, required, excludedIds = []) {
...
export class LocalDuelAdapter {
  constructor(options = {}) {
    ...
    if (options.initialFixture) {
      this.loadFixture(options.initialFixture);
      this.notice = "";
    }
  }
  ...
  loadFixture(name = "default") {
    ...
    this.pendingEvents = [];
    this.privacyRequired = false;
    const fixtureName = String(name || "default");
    const startingPlayer = this.game.priority;
    const defender = startingPlayer === 1 ? 2 : 1;

    const populateLanes = () => {
      for (const lane of this.game.lanes) {
        for (const player of [1, 2]) {
          if (!lane.facedown[player]) lane.facedown[player] = this.game.players[player].hand.shift() || null;
        }
      }
      for (const player of [1, 2]) {
        while (this.game.players[player].hand.length < 8 && this.game.players[player].deck.length) {
          this.game.players[player].hand.push(this.game.players[player].deck.pop());
        }
      }
    };

    const applyFixtureCommand = (player, command) => {
      const result = applyCommand(this.game, {
        ...command,
        player
      });
      if (result.accepted) {
    ...
    if (fixtureName !== "default" && fixtureName !== "opening") {
      populateLanes();
      this.game.turn = 7;
      this.game.players[startingPlayer].life = 34;
      this.game.players[defender].life = 27;
      this.game.message = `Turn 7: Player ${startingPlayer} has priority in a populated review match.`;
    }

    if (["select-attacker", "select-payment", "incoming-hand", "select-blockers"].includes(fixtureName)) {
      const hand = this.game.players[startingPlayer].hand;
      const attacker = hand.slice().sort((left, right) => cardValue(left) - cardValue(right))[0];
      if (fixtureName === "select-attacker" || fixtureName === "select-payment") {
        this.selection = freshSelection({
          kind: "handAttack",
          attackerCardId: attacker.id,
          paymentCardIds: fixtureName === "select-payment"
            ? choosePaymentIds(hand, Math.max(1, cardValue(attacker) - 1), [attacker.id]).slice(0, 1)
            : [],
          selectionRole: "payment"
        });
      } else {
        applyFixtureCommand(startingPlayer, {
          type: "declareHandAttack",
          cardId: attacker.id,
          paymentCardIds: choosePaymentIds(hand, cardValue(attacker), [attacker.id])
        });
        this.game.players[startingPlayer].life = 27;
        this.game.players[defender].life = 34;
        this.controller = defender;
        this.perspective = defender;
        if (fixtureName === "select-blockers") {
          const blockerCardIds = this.game.players[defender].hand
            .slice()
            .sort((left, right) => cardValue(left) - cardValue(right))
            .slice(0, 2)
            .map((card) => card.id);
    ...
    } else if (["lane-attack", "same-lane-block"].includes(fixtureName)) {
      const laneIndex = 1;
      const attacker = this.game.lanes[laneIndex].facedown[startingPlayer];
      applyFixtureCommand(startingPlayer, {
        type: "declareLaneAttack",
        laneIndex,
        paymentCardIds: choosePaymentIds(
          this.game.players[startingPlayer].hand,
          cardValue(attacker)
        )
      });
      this.controller = defender;
      this.perspective = defender;
      if (fixtureName === "same-lane-block") {
        this.selection = freshSelection({
          kind: "laneBlock",
          laneIndex,
          selectionRole: "payment"
    ...
    } else if (fixtureName === "end-placement") {
      applyFixtureCommand(startingPlayer, { type: "passPriority" });
      applyFixtureCommand(defender, { type: "passPriority" });
      this.controller = currentPlacementPlayer(this.game);
      this.perspective = this.controller;
    } else if (fixtureName === "priority-change") {
      applyFixtureCommand(startingPlayer, { type: "passPriority" });
      this.controller = defender;
      this.perspective = defender;
    } else if (fixtureName === "damage-resolution") {
      this.game.players[defender].life = Math.max(0, this.game.players[defender].life - 6);
      this.game.lastEvents = [{
        id: `${this.game.matchId}-fixture-damage`,
        sequence: this.game.eventSequence + 1,
      ...
      this.pendingEvents = this.game.lastEvents;
      this.game.message = `Player ${defender} takes 6 damage.`;
    } else if (fixtureName === "card-draw") {
      const drawnCards = this.game.players[startingPlayer].hand.slice(-2).map((card) => card.id);
      this.game.lastEvents = [{
        id: `${this.game.matchId}-fixture-draw`,
        sequence: this.game.eventSequence + 1,
        revision: this.game.revision,
        type: "cards.drawn",
      ...
      this.pendingEvents = this.game.lastEvents;
      this.game.message = `Player ${startingPlayer} draws to eight cards.`;
    } else if (fixtureName === "constructed-choice") {
      const player = this.game.players[startingPlayer];
      player.faction = { id: "rumin", name: "Rumin" };
      const attacker = player.hand[0];
      const payment = player.hand[1];
      attacker.definitionId = "rumin-forum-ledger-runner";
      attacker.name = "Forum Ledger Runner";
      attacker.type = "unit";
      attacker.factionId = "rumin";
      attacker.value = 3;
      attacker.rank = "3";
      payment.value = 2;
      payment.rank = "2";
      const weapon = this.game.lanes[0].facedown[startingPlayer];
      weapon.definitionId = "rumin-coin-scale-spear";
      weapon.name = "Coin-Scale Spear";
      weapon.type = "weapon";
      weapon.factionId = "rumin";
      this.selection = freshSelection({
        kind: "handAttack",
        attackerCardId: attacker.id,
      ...
      this.game.message = "Constructed choices: select Forum payment and optionally arm the lane weapon.";
    } else if (fixtureName === "faction-ability") {
      const abilityPlayer = [startingPlayer, defender].find((candidate) => (
        this.game.players[candidate]?.faction?.id === "frumo"
      )) || startingPlayer;
      if (abilityPlayer !== this.game.priority) {
        this.game.priority = abilityPlayer;
        this.game.startingPriority = abilityPlayer;
        this.game.priorityPassed = { 1: false, 2: false };
      }
      const ability = getLegalActions(this.game, abilityPlayer)
        .find((action) => action.type === "useFactionAbility");
      if (ability) {
        this.controller = abilityPlayer;
        this.perspective = abilityPlayer;
        this.selection = freshSelection({
          kind: "ability",
          abilityId: ability.abilityId,
          selectionRole: ability.abilityId === "polea-place" || ability.abilityId === "lafayette-swap"
            ? "placement"
            : "primary",
          abilityTargetOwner: ability.abilityId === "polea-peek" ? "opponent" : "local"
        });
        this.game.message = `${ability.label || "Faction ability"}: choose highlighted targets.`;
      }
    } else if (fixtureName === "victory") {
      this.game.phase = "gameOver";
      this.game.winner = startingPlayer;
      this.game.loser = defender;
      this.game.players[defender].life = 0;
      this.game.message = `Player ${startingPlayer} wins the fixture match.`;
      this.game.lastEvents = [{
        id: `${this.game.matchId}-fixture-victory`,
        sequence: this.game.eventSequence + 1,
        revision: this.game.revision,
        type: "match.ended",
      ...
      this.pendingEvents = this.game.lastEvents;
    } else if (fixtureName === "defeat") {
      this.game.phase = "gameOver";
      this.game.winner = defender;
      this.game.loser = startingPlayer;
      this.game.players[startingPlayer].life = 0;
      this.game.message = `Player ${startingPlayer} is defeated.`;
      this.game.lastEvents = [{
        id: `${this.game.matchId}-fixture-defeat`,
        sequence: this.game.eventSequence + 1,
        revision: this.game.revision,
        type: "match.ended",
        winner: defender
      ...
      this.pendingEvents = this.game.lastEvents;
    } else if (fixtureName === "draw") {
      this.game.phase = "gameOver";
      this.game.winner = null;
      this.game.loser = null;
      this.game.players[startingPlayer].life = 0;
      this.game.players[defender].life = 0;
      this.game.message = "Both players reached zero life. The match is a draw.";
      this.game.lastEvents = [{
        id: `${this.game.matchId}-fixture-draw-result`,
        sequence: this.game.eventSequence + 1,
        revision: this.game.revision,
        type: "match.ended",
        winner: null
    ...
    this.controller = nextActor(this.game) || this.controller || startingPlayer;
    if (!this.game.players[this.perspective]) this.perspective = this.controller;
    this.notice = `Loaded visual fixture: ${fixtureName}.`;
    this.emit();
  }
...
export class FixtureMatchAdapter {
  constructor(update) {
    this.update = update;
    this.listeners = new Set();
  }
  ...
  subscribe(listener) {
    this.listeners.add(listener);
    if (this.update) listener(this.update);
    return () => this.listeners.delete(listener);
  }
  ...
  emit() {
    ...
    this.listeners.forEach((listener) => listener(this.update));
  }
  ...
  dispatch(command) {
    return Promise.resolve({
      ...
      revision: this.update?.viewModel?.revision || 0,
      rejection: {
        code: "FIXTURE_READ_ONLY",
        message: "This visual fixture is read-only."
      }
    });
  }
...
export function createLocalDuelAdapter(options) {
  return new LocalDuelAdapter(options);
}

// Production-oriented names. The duel aliases remain until the React
// compatibility renderer and its tests are retired.
export class LocalMatchAdapter extends LocalDuelAdapter {}

export function createLocalMatchAdapter(options) {
  return new LocalMatchAdapter(options);
}

export function createFixtureMatchAdapter(update) {
  return new FixtureMatchAdapter(update);
}
