...
Keep `SUPABASE_SECRET_KEY` on the backend only. Do not put it in Vercel or client code. The legacy `SUPABASE_SERVICE_ROLE_KEY` environment variable remains supported for existing deployments.

The Data API sends new `sb_secret_*` keys in `apikey` only. Supabase Storage follows its server-client contract and sends the backend key in both `apikey` and `Authorization`; keep these request helpers separate when changing key handling.

Player portraits use a private Supabase Storage bucket named `gauntlet-profile-images` by default. The backend service role creates the bucket on the first upload when it does not exist, restricts it to PNG/JPEG/WEBP files up to 1 MB, and proxies public portrait reads through `/api/profiles/:accountId/avatar`. If the project's Storage service or bucket is unavailable, the same private endpoint falls back to durable base64 bytes inside that player's account JSONB; `publicAccountStats` always removes those bytes from API responses. Set `ACCOUNT_AVATAR_BUCKET` only if the project uses a different private bucket name. The browser never receives Supabase credentials, private object paths, or stored portrait bytes. `/api/storage-status` reports `profileImages.available` plus primary-storage readiness without exposing keys or object paths.
